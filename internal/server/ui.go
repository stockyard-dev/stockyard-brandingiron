...
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.type||'').toLowerCase().includes(q)||(i.subject||'').toLowerCase().includes(q)||(i.body||'').toLowerCase().includes(q)});
...
if(i.variables)h+='<span>'+esc(i.variables)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.usage_count)h+='<span>'+esc(i.usage_count)+'</span>';
...
function formHTML(item){var i=item||{name:"",type:"",subject:"",body:"",variables:"",status:"",usage_count:""};var isEdit=!!item;
...
h+='<div class="fr"><label>Variables</label><input id="f-variables" value="'+esc(i.variables)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>Usage Count</label><input id="f-usage-count" value="'+esc(i.usage_count)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),type:document.getElementById('f-type').value.trim(),subject:document.getElementById('f-subject').value.trim(),body:document.getElementById('f-body').value.trim(),variables:document.getElementById('f-variables').value.trim(),status:document.getElementById('f-status').value.trim(),usage_count:document.getElementById('f-usage-count').value.trim()};
